...
export const PROJECT_EMAIL = process.env.PROJECT_EMAIL ?? 'project@albostechnologies.com'
const FROM_EMAIL =
  process.env.RESEND_FROM_EMAIL ?? 'Albos Technologies <noreply@albostechnologies.com>'
...
export async function sendCareerApplicationNotification(data: {
  ...
  customRole?: boolean
  resumeFile?: { filename: string; content: Buffer } | null
}) {
  const html = emailShell(
    ...
    [
      ...
      row('Resume attached', data.resumeFile ? `Yes — ${data.resumeFile.filename}` : 'No'),
    ].join('')
  )
