...
import { isEmailConfigured, sendCareerApplicationNotification } from '@/lib/email'
  ...
  try {
    if (!isEmailConfigured()) {
      return NextResponse.json(
        {
          success: false,
          error: 'Email service is not configured. Please email your application to hr@albostechnologies.com.',
        },
        { status: 503 }
      )
    }
    ...
    const noticePeriod = String(formData.get('noticePeriod') ?? '').trim() || null
    const expectedSalary = String(formData.get('expectedSalary') ?? '').trim() || null
    const source = String(formData.get('source') ?? '').trim() || null
    const coverLetter = String(formData.get('coverLetter') ?? '').trim() || null
    const customRole = formData.get('customRole') === 'true'
    const customRoleTitle = String(formData.get('customRoleTitle') ?? '').trim()
    ...
    if (customRole) {
      if (!customRoleTitle) {
        return NextResponse.json(
          { success: false, error: 'Please specify the role you are applying for' },
          { status: 400 }
        )
      }
    ...
    let resumeFile: { filename: string; content: Buffer } | null = null
    ...
    if (resumeEntry instanceof File && resumeEntry.size > 0) {
      ...
      return NextResponse.json(
        { success: false, error: 'Please upload your resume or provide a resume link' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to send your application. Please try again or email hr@albostechnologies.com.',
      },
      { status: 500 }
    )
  }
