...
import { SectionSkeleton, LoadingSkeleton } from '@/components/ui/LoadingSkeleton';

// Lazy-loaded below-fold sections
const ProjectStorySection = lazy(() =>
  import('@/components/sections/ProjectStorySection').then((m) => ({ default: m.ProjectStorySection }))
);
...
const CTABannerSection = lazy(() =>
  import('@/components/sections/CTABannerSection').then((m) => ({ default: m.CTABannerSection }))
);
...
const CaseStudiesFullSection = lazy(() =>
  import('@/components/sections/CaseStudiesFullSection').then((m) => ({ default: m.CaseStudiesFullSection }))
);
const ContactSection = lazy(() =>
  import('@/components/sections/ContactSection').then((m) => ({ default: m.ContactSection }))
);
const TechnologiesFullSection = lazy(() =>
  import('@/components/sections/TechnologiesFullSection').then((m) => ({ default: m.TechnologiesFullSection }))
);
...
// Blog redirect component — navigates to the /blog route
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA]">
      <div className="text-center">
        <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-[#F97316] border-r-transparent" />
        <p className="mt-4 text-[#52525B] font-[family-name:var(--font-inter)]">Redirecting to blog…</p>
      </div>
    </div>
  );
}

type PageKey = 'home' | 'services' | 'case-studies' | 'about' | 'technologies' | 'industries' | 'blog' | 'contact';
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      <div className="pt-32 pb-20 px-6 md:px-12 lg:px-20">
        <div className="mx-auto max-w-[1400px] space-y-8">
          ...
          <LoadingSkeleton variant="text" className="w-2/3 h-12" />
...
function getPageSkeleton(page: PageKey) {
  switch (page) {
    case 'home': return <HomeSkeleton />;
    case 'services': return <ServicesSkeleton />;
...
const VALID_PAGES: PageKey[] = ['home', 'services', 'case-studies', 'about', 'technologies', 'industries', 'blog', 'contact'];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!currentHash && activePage !== 'home') {
      window.history.replaceState(null, '', `#${activePage}`);
    } else if (currentHash !== activePage && activePage !== 'home') {
      window.history.replaceState(null, '', `#${activePage}`);
    } else if (activePage === 'home' && currentHash) {
          ...
          <AnimatePresence mode="wait">
            {isPageLoading ? (
              ...
              <div
                ...
                style={{ animation: 'fadeIn 0.4s cubic-bezier(0.76, 0, 0.24, 1) forwards' }}
              >
                {activePage === 'home' && (
                  <div
                    key="home"
                ...
                {activePage === 'services' && (
                  ...
                  >
                    <Breadcrumbs items={[{ label: 'Services', page: 'services' }]} onNavigate={handleNavigate} />
                    <Suspense fallback={<SectionFallback />}>
                      <ServicesSection />
                    </Suspense>
                  </motion.div>
                ...
                {activePage === 'case-studies' && (
                  ...
                  >
                    <Breadcrumbs items={[{ label: 'Case Studies', page: 'case-studies' }]} onNavigate={handleNavigate} />
                    <Suspense fallback={<SectionFallback />}>
                ...
                {activePage === 'about' && (
                  ...
                  >
                    <Breadcrumbs items={[{ label: 'About', page: 'about' }]} onNavigate={handleNavigate} />
                    <Suspense fallback={<SectionFallback />}>
                      <AboutSection />
                    </Suspense>
                  </motion.div>
                ...
                {activePage === 'technologies' && (
                  <motion.div
                    ...
                    transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }}
                  >
                    <Breadcrumbs items={[{ label: 'Technologies', page: 'technologies' }]} onNavigate={handleNavigate} />
                    <Suspense fallback={<SectionFallback />}>
                ...
                {activePage === 'industries' && (
                  <motion.div
                    ...
                    transition={{ duration: 0.5, ease: [0.76, 0, 0.24, 1] }}
                  >
                    <Breadcrumbs items={[{ label: 'Industries', page: 'industries' }]} onNavigate={handleNavigate} />
                    <Suspense fallback={<SectionFallback />}>
                      <IndustriesSection />
                    </Suspense>
                  </motion.div>
                ...
                {activePage === 'contact' && (
                  ...
                  >
                    <Breadcrumbs items={[{ label: 'Contact', page: 'contact' }]} onNavigate={handleNavigate} />
