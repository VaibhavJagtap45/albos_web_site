...
type PageKey = 'home' | 'services' | 'case-studies' | 'about' | 'technologies' | 'industries' | 'blog' | 'careers' | 'contact';
...
export function Navbar({ activePage = 'home', onNavigate }: NavbarProps) {
  ...
  const handleNavClick = (href: string) => {
    ...
    // Blog and Careers navigate to their own routes (not SPA state)
    if (href === '#blog') {
      if (activePage === 'blog') {
    ...
    if (page && onNavigate) {
      // SPA navigation within the main page shell
      onNavigate(page);
    } else if (page && !onNavigate) {
      // Standalone pages (Blog/Careers) — navigate to main page with hash
      window.location.href = '/' + href;
    } else if (href === '#home' || !page) {
      if (activePage !== 'home' && onNavigate) {
        onNavigate('home');
      } else if (activePage !== 'home' && !onNavigate) {
        // Standalone pages — navigate back to home
  ...
  const getActiveIndex = () => {
    return NAV_LINKS.findIndex((link) => {
      const linkPage = link.href.replace('#', '') as PageKey;
              ...
              return (
                <li key={link.label} className="relative">
                  <motion.button
                    ...
                    className={cn(
                      "relative inline-block font-[family-name:var(--font-inter)] text-[0.8125rem] font-medium py-1.5 px-3 rounded-md transition-colors duration-300 link-underline",
                      isActive
                        ? "text-[#F97316]"
              ...
              <AnimatePresence>
                {showPhoneTooltip && (
                  ...
                  >
                    ...
                    <p className="text-[0.7rem] text-[#71717A] mt-1">Mon–Sat: 10AM–7PM IST</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            >
              ...
              <div className="flex flex-col h-full pt-[100px] px-8 pb-10">
                        ...
                        >
                          ...
                          >
                            <span className={cn(
                              ...
                              isActive ? "text-[#F97316]" : "text-[#F97316]/60 group-hover:text-[#F97316]"
                            )}>
                            ...
                            <div className="flex-1">
                              <span className={cn(
                                ...
                                isActive ? "text-[#F97316]" : "text-[#18181B] group-hover:text-[#F97316]"
                              )}>
                                {link.label}
                              </span>
                              ...
                              <motion.span
                                ...
                                className={cn(
                                  ...
                                  isActive ? "text-[#F97316]/60" : "text-[#71717A] group-hover:text-[#71717A]"
                                )}
                              >
                                {description}
                              </motion.span>
                            ...
                            <ArrowRight className={cn(
                              ...
                              isActive ? "text-[#F97316] opacity-100" : "text-[#71717A] opacity-0 group-hover:opacity-60 group-hover:translate-x-0.5"
                            )} />
                          </button>
                        </motion.div>
                      );
                ...
                >
                  ...
                  >
                    <span className="text-xs text-[#A1A1AA] font-[family-name:var(--font-inter)]">Follow us</span>
                    <a href={CONTACT_INFO.social.linkedin} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center size-10 rounded-lg bg-black/[0.04] text-[#A1A1AA] hover:text-[#18181B] hover:bg-black/[0.08] transition-all duration-300 hover:scale-110" aria-label="LinkedIn">
                    ...
                    <a href={CONTACT_INFO.social.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center size-10 rounded-lg bg-black/[0.04] text-[#A1A1AA] hover:text-[#18181B] hover:bg-black/[0.08] transition-all duration-300 hover:scale-110" aria-label="Twitter">
                    ...
                    <a href={CONTACT_INFO.social.github} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center size-10 rounded-lg bg-black/[0.04] text-[#A1A1AA] hover:text-[#18181B] hover:bg-black/[0.08] transition-all duration-300 hover:scale-110" aria-label="GitHub">
                    ...
                    <a href={CONTACT_INFO.social.dribbble} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center size-10 rounded-lg bg-black/[0.04] text-[#A1A1AA] hover:text-[#18181B] hover:bg-black/[0.08] transition-all duration-300 hover:scale-110" aria-label="Dribbble">
                      <svg className="size-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 24C5.385 24 0 18.615 0 12S5.385 0 12 0s12 5.385 12 12-5.385 12-12 12zm10.12-10.358c-.35-.11-3.17-.953-6.384-.438 1.34 3.684 1.887 6.684 1.992 7.308 2.3-1.555 3.936-4.02 4.395-6.87zm-6.115 7.808c-.153-.9-.75-4.032-2.19-7.77l-.066.02c-5.79 2.015-7.86 6.025-8.04 6.4 1.73 1.358 3.92 2.166 6.29 2.166 1.42 0 2.77-.29 4-.816zm-11.62-2.58c.232-.4 3.045-5.055 8.332-6.765.135-.045.27-.084.405-.12-.26-.585-.54-1.167-.832-1.74C7.17 11.775 2.206 11.71 1.756 11.7l-.004.312c0 2.633.998 5.037 2.634 6.855zm-2.42-8.955c.46.008 4.683.026 9.477-1.248-1.698-3.018-3.53-5.558-3.8-5.928-2.868 1.35-5.01 3.99-5.676 7.17zM9.6 2.052c.282.38 2.145 2.914 3.822 6 3.645-1.365 5.19-3.44 5.373-3.702C16.86 2.61 14.545 1.62 12 1.62c-.824 0-1.63.1-2.4.285v.146zm10.14 3.2c-.22.305-1.882 2.49-5.663 4.017.226.46.44.93.64 1.4.07.17.14.335.204.5 3.41-.428 6.8.26 7.14.33-.02-2.42-.88-4.64-2.32-6.25z"/></svg>
                    </a>
                  </motion.div>
                </motion.div>
              </div>
