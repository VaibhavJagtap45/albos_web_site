...
const DEPARTMENT_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  Engineering: { bg: 'bg-blue-500/10', text: 'text-blue-600', border: 'border-blue-500/20' },
  Design: { bg: 'bg-pink-500/10', text: 'text-pink-600', border: 'border-pink-500/20' },
  DevOps: { bg: 'bg-amber-500/10', text: 'text-amber-600', border: 'border-amber-500/20' },
  'AI/ML': { bg: 'bg-purple-500/10', text: 'text-purple-600', border: 'border-purple-500/20' },
  'Quality Assurance': { bg: 'bg-emerald-500/10', text: 'text-emerald-600', border: 'border-emerald-500/20' },
  'Business Development': { bg: 'bg-cyan-500/10', text: 'text-cyan-600', border: 'border-cyan-500/20' },
  Marketing: { bg: 'bg-rose-500/10', text: 'text-rose-600', border: 'border-rose-500/20' },
};

const DEFAULT_DEPT_COLOR = { bg: 'bg-[#F97316]/10', text: 'text-[#F97316]', border: 'border-[#F97316]/20' };

/* ─── Filter Options ─── */
const DEPARTMENTS = ['All', 'Engineering', 'Design', 'DevOps', 'AI/ML', 'Quality Assurance', 'Business Development', 'Marketing'] as const;
const JOB_TYPES = ['All Types', 'Full-time', 'Internship', 'Contract'] as const;
const EXPERIENCE_OPTIONS = ['0-1', '1-2', '2-4', '4-6', '6-10', '10+'] as const;
const NOTICE_PERIOD_OPTIONS = ['Immediate', '15 days', '30 days', '60 days', '90 days'] as const;
const SOURCE_OPTIONS = ['LinkedIn', 'Naukri', 'Referral', 'Company Website', 'Other'] as const;
  ...
  return (
    <span
      className={cn(
        ...
        typeColors[type] || 'bg-[#F97316]/10 text-[#F97316] border-[#F97316]/20'
      )}
    >
      {type}
    </span>
...
/* ─── Value Card ─── */
function ValueCard({ value }: { value: (typeof COMPANY_VALUES)[number] }) {
                ...
                <div className="mb-5">
                  ...
                  <ul className="space-y-2">
                    {job.requirements.map((req, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-[#52525B] font-[family-name:var(--font-inter)]">
...
/* ─── Application Form ─── */
function ApplicationForm({ jobs }: { jobs: JobListingData[] }) {
  const [applicationType, setApplicationType] = useState<'listed' | 'custom'>('listed');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!resumeFile && !resumeUrl.trim()) {
      setSubmitError('Please upload your resume (PDF, DOC, or DOCX) or provide a resume link.');
    ...
    try {
      ...
      if (currentCompany) formData.append('currentCompany', currentCompany.trim());
      ...
      if (expectedSalary) formData.append('expectedSalary', expectedSalary.trim());
    ...
    return (
      ...
      >
        ...
        <p className="text-[#52525B] text-sm font-[family-name:var(--font-inter)] max-w-md mx-auto leading-relaxed">
          Thank you for applying{applicationType === 'listed' && selectedJob ? ` for ${selectedJob.title}` : ''}. Our HR team will review your application and get back to you within 5 business days.
        </p>
      </motion.div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white border border-black/[0.06] p-6 md:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {applicationType === 'listed' ? (
          <div>
            <label htmlFor="jobSelect" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
          ...
          <div>
            <label htmlFor="customRoleTitle" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Role you are applying for <span className="text-[#F97316]">*</span>
            </label>
            <input
              id="customRoleTitle"
              type="text"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="fullName" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
          ...
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="phone" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Phone / WhatsApp
            </label>
            <input
              id="phone"
          ...
          <div>
            <label htmlFor="currentCompany" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Current Company
            </label>
            <input
              id="currentCompany"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="experience" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Years of Experience
            </label>
            <select
              id="experience"
          ...
          <div>
            <label htmlFor="linkedinUrl" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
        ...
        {/* Portfolio */}
        <div>
          <label htmlFor="portfolioUrl" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
        ...
        {/* Resume file upload */}
        <div>
          <label htmlFor="resumeFile" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
          ...
          {resumeFileError && (
            <p className="mt-1.5 text-xs text-red-600 font-[family-name:var(--font-inter)]">{resumeFileError}</p>
          )}
          <p className="mt-1.5 text-xs text-[#A1A1AA] font-[family-name:var(--font-inter)]">
            PDF, DOC, or DOCX — max 5 MB. Attached to the email sent to our HR team.
          </p>
        </div>

        {/* Optional resume link */}
        <div>
          <label htmlFor="resumeUrl" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="noticePeriod" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Notice Period
            </label>
            <select
              id="noticePeriod"
          ...
          <div>
            <label htmlFor="expectedSalary" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
              Expected Salary
            </label>
            <input
              id="expectedSalary"
        ...
        {/* Source */}
        <div>
          <label htmlFor="source" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
        ...
        {/* Cover Letter */}
        <div>
          <label htmlFor="coverLetter" className="block text-sm font-medium text-[#18181B] font-[family-name:var(--font-inter)] mb-1.5">
            Cover Letter
          </label>
          <textarea
            id="coverLetter"
        ...
        <p className="text-center text-[11px] text-[#A1A1AA] font-[family-name:var(--font-inter)] leading-relaxed">
          By submitting, you agree to our privacy policy. Your data will be used solely for recruitment purposes and will not be shared with third parties.
        </p>
      </form>
    </div>
  );
  ...
  return (
    <div className="py-20 text-center">
      ...
      <p className="text-[#52525B] font-[family-name:var(--font-inter)] text-sm max-w-md mx-auto">
        No open positions match your current filters. Try adjusting the department or job type, or check back soon for new openings.
      </p>
    </div>
  );
}
...
export function CareersPageClient({ jobs }: { jobs: JobListingData[] }) {
  const [activeDepartment, setActiveDepartment] = useState<string>('All');
  const [activeJobType, setActiveJobType] = useState<string>('All Types');
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const typeMatch = activeJobType === 'All Types' || job.type === activeJobType;
          ...
          <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
            ...
            <nav aria-label="Breadcrumb" className="mb-8">
              <ol className="flex items-center gap-2 text-sm font-[family-name:var(--font-inter)]">
                <li>
                  <Link href="/" className="text-[#A1A1AA] hover:text-[#F97316] transition-colors duration-300">
                    Home
                  </Link>
                </li>
            ...
            <p className="mt-6 text-[#A1A1AA] text-lg md:text-xl font-[family-name:var(--font-inter)] max-w-2xl leading-relaxed">
              Join Albos Technologies Pvt Ltd and shape the next generation of enterprise software. Work on meaningful projects with a team that values craft, ownership, and growth.
            </p>
        ...
        <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
          ...
          <div className="text-center mb-12 md:mb-16">
            ...
            <p className="mt-4 text-[#52525B] text-base md:text-lg font-[family-name:var(--font-inter)] max-w-2xl mx-auto leading-relaxed">
              Six principles that define how we build, collaborate, and grow — every single day.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <div>
              ...
              <p className="mt-4 text-[#52525B] text-base font-[family-name:var(--font-inter)] leading-relaxed">
                Fill out the form and our talent acquisition team will review your application. We typically respond within 5 business days.
              </p>
