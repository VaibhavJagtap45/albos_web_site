...
export function HeroCanvas() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const gradient1 = ctx.createRadialGradient(orbX1, orbY1, 0, orbX1, orbY1, 350);
      ...
      const gradient2 = ctx.createRadialGradient(orbX2, orbY2, 0, orbX2, orbY2, 280);
      ...
      const projectVert = (v: [number, number, number]): { x: number; y: number; z: number } => {
      ...
      for (const v of projectedVerts) {
        ...
        const glowGrad = ctx.createRadialGradient(v.x, v.y, 0, v.x, v.y, dotSize * 3);
        glowGrad.addColorStop(0, `rgba(249, 115, 22, ${Math.min(opacity * 0.4, 0.3)})`);
      ...
      for (const i of sortedIndices) {
        ...
        if (p.isAccent) {
          ...
          const accentGlow = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, size * 4);
          accentGlow.addColorStop(0, `rgba(249, 115, 22, ${depthOpacity * 0.25})`);
          ...
          const whiteGlow = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, size * 2.5);
          whiteGlow.addColorStop(0, `rgba(120, 113, 108, ${depthOpacity * 0.12})`);
