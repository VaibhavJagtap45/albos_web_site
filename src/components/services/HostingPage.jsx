...
//         icon="https://cdn-icons-png.flaticon.com/512/3103/3103446.png"
//         features={["5 Websites","50 GB SSD Storage","Free SSL + Daily Backup","Premium Malware Scan","Free Domain for 1 Year"]}/>
//       <PricingCard delay={160} title="Premium"  price="₹399/mo" accent="#1d4ed8"
//         icon="https://cdn-icons-png.flaticon.com/512/1048/1048903.png"
//         features={["Unlimited Websites","100 GB NVMe SSD","Free CDN + WAF Firewall","Priority Support","Staging + Git Deployment"]}/>
//     </div>
//   </Section>
...
//         icon="https://cdn-icons-png.flaticon.com/512/236/236831.png"
//         features={["Intel Xeon E3","16 GB RAM","2×512 GB SSD","1 Gbps Port","Full Root Access"]}/>
//       <PricingCard delay={80}  title="Intel Xeon Pro"        price="₹7,999/mo"  accent="#1d4ed8" popular
//         icon="https://cdn-icons-png.flaticon.com/512/201/201623.png"
//         features={["Xeon E5 Dual","32 GB RAM","2×1 TB NVMe","DDoS Protection","Managed Firewall"]}/>
//       <PricingCard delay={160} title="AMD EPYC Enterprise"   price="₹12,999/mo" accent="#be185d"
//         icon="https://cdn-icons-png.flaticon.com/512/3208/3208753.png"
...
/* ─── hook ─────────────────────────────────────────────── */
function useRv(thr = 0.08) {
  const ref = useRef(null);
  useEffect(() => {
    const el = ref.current; if (!el) return;
    // rootMargin pulls trigger point 80px above the bottom edge —
    // ensures footer elements animate in even if only partially visible
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) { el.classList.add("on"); obs.disconnect(); }
    }, { threshold: thr, rootMargin: "0px 0px 80px 0px" });
    obs.observe(el);
    // Fallback: if still not triggered after 1.2s, reveal anyway
    const timer = setTimeout(() => { el.classList.add("on"); }, 1200);
...
/* ─── domain pill — own component so useRv is at top level ─ */
function DomainPill({ tld, price, color, delay }) {
  const r = useRv();
  return (
    <div ref={r} className="rv domain-pill" style={{ transitionDelay:`${delay}ms` }}>
      <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:"1.7rem", fontWeight:400, color }}>{tld}</div>
      <div style={{ fontSize:".85rem", fontWeight:700, color:"var(--blue)", marginTop:6 }}>{price}</div>
      <button
        style={{
          marginTop:12, padding:"6px 16px", borderRadius:8,
          background:color+"14", color, border:`1px solid ${color}30`,
          fontSize:".72rem", fontWeight:700, cursor:"pointer", transition:"background .2s",
        }}
        onMouseEnter={e => e.currentTarget.style.background = color+"2a"}
        onMouseLeave={e => e.currentTarget.style.background = color+"14"}
      >Register</button>
  ...
  return (
    <section style={{ padding:"88px 24px", background:"var(--bg)" }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:14 }}>
          {tlds.map(([tld, price, color], i) => (
            <DomainPill key={tld} tld={tld} price={price} color={color} delay={i * 55} />
          ))}
        </div>
      </div>
    </section>
...
/* ─── contact data — defined OUTSIDE component (stable, no re-creation) ─ */
const CONTACT_ITEMS = [
  { label:"Phone",        val:"+91 97666 50411",      icon:"phone"   },
  { label:"Email",        val:"info@albostechnologies.com", icon:"mail" },
  { label:"Location",     val:"Office 78, 5th Floor, Kunal Plaza, Pune 411019", icon:"pin" },
  { label:"Office Hours", val:"Mon – Sat, 10:00 AM – 7:00 PM", icon:"clock" },
];

function ContactIcon({ type }) {
  const props = { xmlns:"http://www.w3.org/2000/svg", width:22, height:22, fill:"none", viewBox:"0 0 24 24", stroke:"currentColor", strokeWidth:1.8 };
  if (type === "phone") return <svg {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384"/></svg>;
  if (type === "mail")  return <svg {...props}><path strokeLinecap="round" strokeLinejoin="round" d="m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7"/><rect x="2" y="4" width="20" height="16" rx="2"/></svg>;
  if (type === "pin")   return <svg {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>;
  return <svg {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2"/><circle cx="12" cy="12" r="10"/></svg>;
}

/* ─── contact card — own component so useRv is at top level ─ */
function ContactCard({ label, val, icon, delay }) {
  const r = useRv();
  return (
    <div ref={r} className="rv c-card" style={{ transitionDelay:`${delay}ms` }}>
      <div className="c-icon"><ContactIcon type={icon} /></div>
      <div>
        <div style={{ fontWeight:700, fontSize:".88rem", marginBottom:4 }}>{label}</div>
        <div style={{ fontSize:".83rem", color:"var(--mid)", lineHeight:1.6 }}>{val}</div>
      </div>
  ...
  return (
    <section style={{ padding:"88px 24px 96px", background:"var(--white)" }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        ...
        {/* contact cards */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
          {CONTACT_ITEMS.map(({ label, val, icon }, i) => (
            <ContactCard key={label} label={label} val={val} icon={icon} delay={i * 70} />
          ))}
        </div>
      </div>
    </section>
