...
import { X, Clock, User, ArrowLeft, Linkedin, Link as LinkIcon, Check } from 'lucide-react';
import { useEffect, useState } from 'react';
import { BLOG_POSTS } from '@/lib/constants';

/* ─── Category gradient map ─── */
const categoryGradients: Record<string, string> = {
  Engineering: 'linear-gradient(135deg, #1a0800 0%, #2a1500 40%, #0d0d0d 70%, #1a0a00 100%)',
  'AI/ML': 'linear-gradient(135deg, #0d0014 0%, #1a0033 40%, #0d0d0d 70%, #0a0014 100%)',
  Design: 'linear-gradient(135deg, #001a0d 0%, #00331a 40%, #0d0d0d 70%, #001a10 100%)',
  Business: 'linear-gradient(135deg, #1a0f00 0%, #332000 40%, #0d0d0d 70%, #1a1200 100%)',
  'Case Studies': 'linear-gradient(135deg, #0a0014 0%, #1a002e 40%, #0d0d0d 70%, #14001a 100%)',
};
...
const categoryTags: Record<string, string[]> = {
  Engineering: ['Kubernetes', 'Microservices', 'Docker', 'DevOps', 'Cloud Native'],
  'AI/ML': ['LLMs', 'Generative AI', 'Machine Learning', 'NLP', 'Python'],
  Design: ['Design Systems', 'Figma', 'Component Library', 'Accessibility', 'UX'],
  Business: ['Cloud Migration', 'Digital Transformation', 'CTO', 'Strategy', 'ROI'],
  'Case Studies': ['HIPAA', 'Healthcare', 'Compliance', 'Telehealth', 'Security'],
};

interface BlogArticleModalProps {
  isOpen: boolean;
  onClose: () => void;
  article: (typeof BLOG_POSTS)[number] | null;
}
...
export function BlogArticleModal({ isOpen, onClose, article }: BlogArticleModalProps) {
  ...
  const gradient = categoryGradients[article.category] || categoryGradients.Engineering;
  const bodyParagraphs = articleBodies[article.category] || articleBodies.Engineering;
          ...
          >
            ...
            <div className="relative h-56 md:h-72 overflow-hidden rounded-t-2xl">
              ...
              <div
                className="absolute inset-0"
                style={{
                  background: 'radial-gradient(ellipse 50% 60% at 60% 40%, rgba(249, 115, 22, 0.15), transparent)',
                }}
              />
              {/* Header accent line */}
