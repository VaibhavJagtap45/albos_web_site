...
const FILTER_CATEGORIES = ['All', 'HealthTech', 'FinTech', 'EdTech', 'Retail', 'AI/ML', 'Logistics'] as const;
...
function StudyCard({ study, large = false }: { study: CaseStudyData; large?: boolean }) {
  return (
    <Link
    ...
    >
      <div className={cn('relative w-full', large ? 'aspect-[21/9]' : 'aspect-[16/10]')}>
        <Image
          src={study.image}
          alt={study.title}
          fill
...
export function CaseStudiesPageClient({ studies }: { studies: CaseStudyData[] }) {
        ...
        <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
          <ScrollReveal direction="up">
            ...
            <p className="mt-4 text-[#71717A] text-lg max-w-2xl font-[family-name:var(--font-inter)]">
              Real results from real partnerships — explore how Albos Technologies Pvt Ltd delivers measurable outcomes.
            </p>
          </ScrollReveal>
