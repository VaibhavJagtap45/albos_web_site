...
import { SingleDonutChart, MultiDonutChart } from '@/components/ui/AnimatedDonutChart';
import { cn } from '@/lib/utils';

type CaseStudy = (typeof CASE_STUDIES)[number];

const FILTER_CATEGORIES = ['All', 'HealthTech', 'FinTech', 'EdTech', 'Retail', 'AI/ML', 'Mobile'] as const;
...
function FeaturedCard({ study }: { study: CaseStudy }) {
  return (
    ...
    >
    <motion.div
      whileHover={{ scale: 1.005, transition: { duration: 0.4, ease: [0.16, 1, 0.3, 1] } }}
    >
...
function GridCard({ study, tall = false }: { study: CaseStudy; tall?: boolean }) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        tall ? 'min-h-[360px] md:min-h-[440px]' : 'min-h-[280px] md:min-h-[340px]'
      )}
  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={cn('h-3.5 w-3.5', i < rating ? 'text-[#F97316] fill-[#F97316]' : 'text-white/15 fill-[#FAFAFA]/15')}
        />
      ))}
    </div>
  );
...
export function CaseStudiesFullSection() {
  ...
  return (
    <section id="case-studies-full" className="relative bg-[#FAFAFA] mesh-gradient-2 overflow-hidden">
      {/* ─── Hero ─── */}
        ...
        <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
          <ScrollReveal direction="up">
            ...
            <h1 className="mt-4 font-[family-name:var(--font-plus-jakarta)] text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-albos-text leading-[1.05]">
              500+ Projects.<br />Zero Compromises.
            </h1>
          </ScrollReveal>
        </div>
      </div>
            ...
            <AnimatePresence mode="popLayout">
              {filteredStudies.map((study, i) => (
                <motion.div
                  ...
                  className={cn(
                    tallIndices.includes(i) ? 'sm:row-span-2' : ''
                  )}
                >
        ...
        <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
          <ScrollReveal direction="up" className="mb-10 md:mb-14">
            ...
            <p className="mt-3 text-[#71717A] text-sm md:text-base font-[family-name:var(--font-inter)] text-center max-w-lg mx-auto">
              Our track record speaks for itself. Consistent results across every engagement.
            </p>
          </ScrollReveal>
