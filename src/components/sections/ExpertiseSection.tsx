...
export function ExpertiseSection() {
  ...
  const scrollToCard = useCallback((index: number) => {
    ...
    const scrollTarget = pinTrigger.start + (pinTrigger.end - pinTrigger.start) * targetProgress;
      ...
      <div className="mx-auto max-w-[var(--container-max)] px-6 md:px-12 lg:px-20">
        ...
        <div className="lg:hidden">
          <div className="sticky top-16 z-30 -mx-6 px-6 py-3 bg-[#FAFAFA]/95 backdrop-blur-xl border-b border-black/[0.06] mb-6">
            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
              {SERVICES.map((service, i) => (
                ...
                >
                  <span className="text-albos-accent/60 mr-1.5">{service.id}</span>
                  {service.title}
                </button>
              ))}
            </div>
          ...
          {SERVICES.map((service, i) => (
            ...
            >
              <ServiceCardContent service={service} isActive={i === activeIndex} />
            </div>
          ))}
        </div>
            ...
            <div className="sticky top-28">
              ...
              >
                ...
                <p className="mt-4 text-albos-muted text-sm leading-relaxed font-[family-name:var(--font-inter)] max-w-xs">
                  Six core disciplines. One integrated team. End-to-end delivery from strategy to scale.
                </p>
              </motion.div>
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      <span className={cn(
                        ...
                        i === activeIndex ? 'text-albos-dark'
                          : i < activeIndex ? 'text-albos-accent'
                    ...
                    <div className="relative z-10 min-w-0">
                      <span className={cn(
                        ...
                        i === activeIndex ? 'text-albos-accent' : 'text-albos-text/70 group-hover:text-albos-text'
                      )}>{service.title}</span>
                      <AnimatePresence>
                        {i === activeIndex && (
                          <motion.span
                ...
                {SERVICES.map((_, i) => (
                  <motion.div
                    ...
                    className={cn(
                      ...
                      i === activeIndex ? 'w-6 bg-albos-accent'
                        : i < activeIndex ? 'w-3 bg-albos-accent/30'
                        : 'w-3 bg-black/[0.1]'
                    )}
            ...
            >
              {SERVICES.map((service, i) => (
                <div
                  ...
                  style={{ height: '75vh' }}
                >
                  <ServiceCardContent service={service} isActive={activeIndex === i} />
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className={cn(
        ...
        {isActive && (
          <span className="ml-auto text-[10px] text-albos-accent/60 font-[family-name:var(--font-inter)]">{service.metric}</span>
        )}
      </div>
      ...
      <div className="relative z-10 p-5 sm:p-7 md:p-8 lg:p-10 bg-white flex-1 overflow-y-auto">
        ...
        <div className="mt-6">
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <span className="text-xs uppercase tracking-[0.15em] text-albos-muted font-[family-name:var(--font-inter)]">Offices</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {service.locations.map((loc, j) => (
              <span key={loc} className={cn(
                ...
                j === 0 ? 'bg-albos-accent text-albos-dark'
        ...
        <div className="mt-6">
          <span className="text-xs uppercase tracking-[0.15em] text-albos-muted font-[family-name:var(--font-inter)] mb-2.5 block">Capabilities</span>
          <div className="flex flex-wrap gap-2">
            {service.capabilities.map((cap) => (
              <span key={cap} className="rounded-full border border-black/[0.08] bg-black/[0.03] px-3 py-1 text-[11px] font-medium text-albos-text/70 font-[family-name:var(--font-inter)] hover:border-albos-accent/30 hover:text-albos-text transition-all duration-300">{cap}</span>
            ))}
          </div>
        </div>

        <div className="mt-6">
          <span className="text-xs uppercase tracking-[0.15em] text-albos-muted font-[family-name:var(--font-inter)] mb-2.5 block">Tech Stack</span>
          <div className="flex flex-wrap gap-2">
            {service.techStack.map((tech) => (
              <span key={tech} className="inline-flex items-center gap-1.5 rounded-md bg-black/[0.04] border border-black/[0.06] px-2.5 py-1.5 text-[11px] font-mono text-albos-text/60 hover:text-albos-accent hover:border-albos-accent/30 transition-all duration-300">
                <span className="h-1.5 w-1.5 rounded-full bg-albos-accent/60" />{tech}
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8">
          <span className="group inline-flex items-center gap-2 text-albos-accent font-[family-name:var(--font-inter)] text-sm font-semibold transition-colors duration-300 hover:text-albos-accent-hover cursor-pointer">
            Learn More
