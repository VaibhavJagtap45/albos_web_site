...
const PROJECTS = [
  {
    ...
    title: 'FinTrack Pro',
    category: 'FinTech',
    description: 'Real-time financial analytics platform processing 10M+ transactions daily with sub-100ms latency.',
    result: '40% faster load time',
    techStack: ['React', 'Node.js', 'PostgreSQL', 'AWS'],
    image: '/images/projects/fintrack-pro.jpg',
    accentColor: '#F97316',
    accentGlow: 'rgba(249, 115, 22, 0.15)',
    bgShift: 'radial-gradient(ellipse 80% 60% at 30% 40%, rgba(249, 115, 22, 0.08), transparent)',
    metric: '10M+',
  ...
  },
  {
    ...
    title: 'MedConnect',
    category: 'HealthTech',
    description: 'HIPAA-compliant telehealth platform connecting 50K+ patients with healthcare providers.',
    result: '3x patient engagement',
    techStack: ['Next.js', 'Python', 'MongoDB', 'Azure'],
    image: '/images/projects/medconnect.jpg',
    accentColor: '#10B981',
    accentGlow: 'rgba(16, 185, 129, 0.12)',
    bgShift: 'radial-gradient(ellipse 80% 60% at 60% 50%, rgba(16, 185, 129, 0.08), transparent)',
    metric: '50K+',
  ...
  },
  {
    ...
    title: 'EduSpark',
    category: 'EdTech',
    description: 'AI-powered adaptive learning platform serving 200K+ students across 15 countries.',
    ...
    bgShift: 'radial-gradient(ellipse 80% 60% at 50% 60%, rgba(168, 85, 247, 0.08), transparent)',
    metric: '200K+',
  ...
  },
  {
    ...
    title: 'RetailOS',
    category: 'Retail',
    description: 'Omnichannel retail management system handling $2B+ annual transactions.',
    result: '2x revenue growth',
    techStack: ['Next.js', 'Python', 'Redis', 'AWS'],
    image: '/images/projects/retailos.jpg',
    accentColor: '#F59E0B',
    accentGlow: 'rgba(245, 158, 11, 0.12)',
    bgShift: 'radial-gradient(ellipse 80% 60% at 40% 50%, rgba(245, 158, 11, 0.08), transparent)',
    metric: '$2B+',
  ...
  },
  {
    ...
    title: 'DataVault',
    category: 'AI/ML',
    description: 'Enterprise data governance platform with automated ML pipeline management.',
    result: '80% less manual work',
    techStack: ['React', 'Python', 'Kubernetes', 'Snowflake'],
    image: '/images/projects/datavault.jpg',
    accentColor: '#F97316',
    accentGlow: 'rgba(249, 115, 22, 0.15)',
    bgShift: 'radial-gradient(ellipse 80% 60% at 50% 40%, rgba(249, 115, 22, 0.12), transparent)',
    metric: '80%',
...
// ─── SceneProgress ────────────────────────────────────────────────────────
function SceneProgress({ activeIndex, progress }: { activeIndex: number; progress: number }) {
  return (
    <div className="absolute right-4 md:right-8 lg:right-12 top-1/2 -translate-y-1/2 z-30 flex flex-col items-center gap-3">
      {PROJECTS.map((project, i) => (
        <div key={project.id} className="flex items-center gap-2 group/progress">
          <span
            ...
            style={{ color: i === activeIndex ? project.accentColor : '#A1A1AA' }}
          >
            {project.category}
          </span>
          <div className="relative flex items-center justify-center">
            <div
              ...
              style={{
                ...
                backgroundColor: i === activeIndex ? project.accentColor : 'rgba(255,255,255,0.15)',
                boxShadow: i === activeIndex ? `0 0 12px ${project.accentColor}40, 0 0 24px ${project.accentColor}20` : 'none',
              }}
            />
          </div>
        </div>
...
// ─── BackgroundLayer ──────────────────────────────────────────────────────
function BackgroundLayer({ activeIndex }: { activeIndex: number }) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ opacity: 0, willChange: 'transform', filter: isActive ? 'blur(0px)' : 'blur(8px)', transition: 'filter 0.6s ease' }}
      >
        <div
      ...
      <motion.div
        ...
        animate={isActive ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: 60, scale: 0.9 }}
        transition={{ delay: 0.5, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
      >
        <div className="glass-card px-4 md:px-5 py-3 md:py-4 min-w-[120px] md:min-w-[140px]" style={{ animation: 'floatCard1 6s ease-in-out infinite' }}>
          <div
      ...
      >
        <div className="glass-card px-4 py-3" style={{ animation: 'floatCard2 7s ease-in-out infinite' }}>
    ...
    >
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden">
        ...
        <motion.div
          ...
          initial={{ opacity: 0, y: 20 }}
          animate={{
            opacity: activeScene === PROJECTS.length - 1 && sceneProgress > 0.7 ? 1 : 0,
            y: activeScene === PROJECTS.length - 1 && sceneProgress > 0.7 ? 0 : 20,
          }}
        ...
        <div className="absolute bottom-8 left-6 md:left-12 lg:left-20 z-30 flex items-center gap-3">
          ...
          <span className="text-[10px] font-[family-name:var(--font-jetbrains-mono)] text-albos-muted tabular-nums">
            {String(activeScene + 1).padStart(2, '0')} / {String(PROJECTS.length).padStart(2, '0')}
          </span>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {PROJECTS.map((project) => (
              <div key={project.id} className="glass-card p-6 rounded-2xl">
                ...
                <p className="mt-2 text-albos-muted text-sm">{project.description}</p>
                <div className="mt-3 flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: project.accentColor }} />
                  <span className="text-sm font-semibold text-white/80">{project.result}</span>
                </div>
              </div>
            ))}
          </div>
