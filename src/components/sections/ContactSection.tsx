...
function OfficeMap() {
  ...
  const officePositions = [
    { city: 'Pune', x: 680, y: 200, teamSize: 250 },
  ];

  return (
    <div ref={ref} className="relative rounded-xl overflow-hidden border border-black/[0.06] bg-[#FAFAFA]">
      <svg
      ...
      >
        ...
        <g opacity="0.03">
          {Array.from({ length: 21 }, (_, i) => (
            <line key={`v${i}`} x1={i * 50} y1="0" x2={i * 50} y2="500" stroke="black" strokeWidth="0.5" />
          ))}
          {Array.from({ length: 11 }, (_, i) => (
            <line key={`h${i}`} x1="0" y1={i * 50} x2="1000" y2={i * 50} stroke="black" strokeWidth="0.5" />
          ))}
        </g>
        <motion.path
        ...
        {officePositions.map((office) => (
          <g key={office.city}>
            <motion.circle cx={office.x} cy={office.y} r="12" fill="none" stroke="#F97316" strokeWidth="1"
              initial={{ opacity: 0, scale: 0 }}
              animate={isInView ? { opacity: [0, 0.4, 0], scale: [0.5, 1.5, 2] } : {}}
              transition={{ duration: 2.5, repeat: Infinity, ease: 'easeOut', delay: 1.5 }}
            />
            <motion.circle cx={office.x} cy={office.y} r="8" fill="#F97316" opacity={0.1}
              initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}}
              transition={{ duration: 0.4, delay: 1.3 }}
            />
            <motion.circle cx={office.x} cy={office.y} r="4" fill="#F97316"
              initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}}
            ...
            />
            <motion.circle cx={office.x} cy={office.y} r="1.5" fill="#FFB380"
              initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}}
              transition={{ duration: 0.3, delay: 1.5 }}
            />
            {hoveredOffice === office.city && (
              <g>
                <rect x={office.x - 55} y={office.y - 42} width="110" height="32" rx="6" fill="#FFFFFF" stroke="rgba(249,115,22,0.3)" strokeWidth="0.5" />
                <text x={office.x} y={office.y - 26} textAnchor="middle" fill="#18181B" fontSize="9" fontFamily="var(--font-plus-jakarta)" fontWeight="700">{office.city}</text>
                <text x={office.x} y={office.y - 15} textAnchor="middle" fill="#F97316" fontSize="7" fontFamily="var(--font-inter)">{office.teamSize} members</text>
              </g>
            )}
          </g>
        ))}
        <g opacity="0.4">
          ...
          <text x="880" y="453" fill="#A1A1AA" fontSize="8" fontFamily="var(--font-inter)">Office Location</text>
        </g>
      </svg>
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-4">
      <a href={CONTACT_INFO.social.linkedin} target="_blank" rel="noopener noreferrer"
        className="flex items-center justify-center w-10 h-10 rounded-full border border-black/[0.08] text-[#A1A1AA] hover:text-[#F97316] hover:border-[#F97316]/30 transition-all duration-300" aria-label="LinkedIn">
        <Linkedin className="h-4 w-4" />
      </a>
      <a href={CONTACT_INFO.social.twitter} target="_blank" rel="noopener noreferrer"
        className="flex items-center justify-center w-10 h-10 rounded-full border border-black/[0.08] text-[#A1A1AA] hover:text-[#F97316] hover:border-[#F97316]/30 transition-all duration-300" aria-label="Twitter / X">
        <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" role="img" aria-hidden="true">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      </a>
      <a href={CONTACT_INFO.social.github} target="_blank" rel="noopener noreferrer"
        className="flex items-center justify-center w-10 h-10 rounded-full border border-black/[0.08] text-[#A1A1AA] hover:text-[#F97316] hover:border-[#F97316]/30 transition-all duration-300" aria-label="GitHub">
        <Github className="h-4 w-4" />
      </a>
      <a href={CONTACT_INFO.social.dribbble} target="_blank" rel="noopener noreferrer"
        className="flex items-center justify-center w-10 h-10 rounded-full border border-black/[0.08] text-[#A1A1AA] hover:text-[#F97316] hover:border-[#F97316]/30 transition-all duration-300" aria-label="Dribbble">
        <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" role="img" aria-hidden="true">
...
function ContactMethodCard({
  icon, label, value, href, subtext,
}: {
  icon: React.ReactNode; label: string; value: string; href?: string; subtext?: string;
}) {
  const content = (
...
/* ─── Form Input ─── */
function FormInput({
  label, type = 'text', value, onChange, placeholder, error, required = false, id,
}: {
  label: string; type?: string; value: string; onChange: (v: string) => void;
  placeholder?: string; error?: string; required?: boolean; id?: string;
}) {
  const inputId = id || label.toLowerCase().replace(/\s+/g, '-');
  return (
    <div className="space-y-1.5">
      <label htmlFor={inputId} className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#52525B] font-[family-name:var(--font-inter)]">
        {label} {required && <span className="text-[#F97316]">*</span>}
      </label>
      <input
        id={inputId} type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
        className={cn(
      ...
      />
      {error && <p className="text-xs text-red-500 font-[family-name:var(--font-inter)]">{error}</p>}
    </div>
  );
}
...
/* ─── Form Select ─── */
function FormSelect({
  label, value, onChange, placeholder, error, required = false, id, options,
}: {
  label: string; value: string; onChange: (v: string) => void;
  placeholder?: string; error?: string; required?: boolean; id?: string; options: string[];
}) {
  const inputId = id || label.toLowerCase().replace(/\s+/g, '-');
  ...
  return (
    <div className="space-y-1.5 relative" ref={ref}>
      ...
      >
        ...
        <ChevronDown className={cn('size-4 text-[#9CA3AF] transition-transform duration-200', isOpen && 'rotate-180')} />
      </button>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            {options.map((option) => (
              <button
                ...
                className={cn(
                  ...
                  value === option ? 'bg-[#F97316]/10 text-[#F97316] font-semibold' : 'text-[#18181B]'
                )}
              >
                {option}
              </button>
...
function FormTextarea({
  label, value, onChange, placeholder, error, required = false, id, rows = 4,
}: {
  label: string; value: string; onChange: (v: string) => void;
  placeholder?: string; error?: string; required?: boolean; id?: string; rows?: number;
}) {
  const inputId = id || label.toLowerCase().replace(/\s+/g, '-');
  return (
    <div className="space-y-1.5">
      ...
      <textarea
        id={inputId} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={rows}
        className={cn(
...
function FAQItem({
  ...
  return (
    <div className={cn('border-b border-black/[0.06] transition-all duration-300', isOpen && 'border-l-2 border-l-[#F97316]')}>
      <button onClick={onToggle} className="flex w-full items-center justify-between py-5 md:py-6 text-left group pl-4 md:pl-5">
        <span className={cn(
          'font-[family-name:var(--font-plus-jakarta)] text-base md:text-lg font-semibold pr-4 transition-colors duration-300',
          isOpen ? 'text-[#18181B]' : 'text-[#18181B]/80 group-hover:text-[#18181B]'
        )}>
          {question}
        </span>
        <motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="shrink-0">
          <ChevronDown className={cn('h-5 w-5 transition-colors duration-300', isOpen ? 'text-[#F97316]' : 'text-[#A1A1AA] group-hover:text-[#F97316]/60')} />
        </motion.div>
      </button>
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }} className="overflow-hidden">
            <p className="pb-5 md:pb-6 pl-4 md:pl-5 text-[#71717A] text-sm md:text-base leading-relaxed font-[family-name:var(--font-inter)] max-w-2xl">{answer}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="divide-y divide-white/[0.06]">
      {FAQ_ITEMS.map((item, i) => (
        <FAQItem key={i} question={item.q} answer={item.a} isOpen={openIndex === i} onToggle={() => setOpenIndex(openIndex === i ? null : i)} />
      ))}
    </div>
  );
}
...
/* ─── Main Section ─── */
export function ContactSection({ onNavigate }: { onNavigate?: (page: string) => void }) {
    ...
    <section id="contact" className="relative overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="relative bg-white w-full lg:w-7/12 py-16 md:py-20 lg:py-36 px-6 md:px-12 lg:px-16 xl:px-20">
          ...
          <div className="relative z-10 max-w-2xl mx-auto lg:mx-0" data-contact-form>
            {!isSubmitted ? (
              <motion.form
              ...
              >
                ...
                <p className="mt-3 text-[#71717A] text-base font-[family-name:var(--font-inter)] max-w-md">
                  Thank you, {form.name || 'there'}! Our team will review your project details and reach out within 24 hours.
                </p>
        ...
        <div className="mx-auto max-w-3xl">
          <ScrollReveal>
            ...
            <p className="mt-3 text-[#71717A] text-base font-[family-name:var(--font-inter)] text-center max-w-xl mx-auto">
              Everything you need to know about working with Albos Technologies Pvt Ltd.
            </p>
          </ScrollReveal>
          <div className="mt-10">
            <FAQAccordion />
