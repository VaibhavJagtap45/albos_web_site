...
const showcaseData: ShowcaseCategory[] = [
  {
    ...
    label: 'BRANDING',
    cards: [
      { image: '/images/showcase/branding-1.png', rotation: -6, scale: 0.85, isLandscape: true },
      { image: '/images/showcase/branding-2.png', rotation: 4, scale: 0.9, isLandscape: false },
      { image: '/images/showcase/branding-3.png', rotation: -3, scale: 0.82, isLandscape: true },
      { image: '/images/showcase/branding-4.png', rotation: 5, scale: 0.88, isLandscape: false },
    ],
  },
  {
    key: 'design',
    label: 'DESIGN',
    cards: [
      { image: '/images/showcase/design-1.png', rotation: 3, scale: 0.9, isLandscape: true },
      { image: '/images/showcase/design-2.png', rotation: -5, scale: 0.85, isLandscape: false },
      { image: '/images/showcase/design-3.png', rotation: -2, scale: 0.83, isLandscape: true },
      { image: '/images/showcase/design-4.png', rotation: 4, scale: 0.87, isLandscape: false },
    ],
  },
  {
    key: 'development',
    label: 'DEVELOPMENT',
    cards: [
      { image: '/images/showcase/development-1.png', rotation: -4, scale: 0.88, isLandscape: true },
      { image: '/images/showcase/development-2.png', rotation: 5, scale: 0.82, isLandscape: false },
      { image: '/images/showcase/development-3.png', rotation: 2, scale: 0.84, isLandscape: true },
      { image: '/images/showcase/development-4.png', rotation: -3, scale: 0.86, isLandscape: false },
    ],
  },
  {
    key: 'strategy',
    label: 'STRATEGY',
    cards: [
      { image: '/images/showcase/strategy-1.png', rotation: 4, scale: 0.85, isLandscape: true },
      { image: '/images/showcase/strategy-2.png', rotation: -3, scale: 0.9, isLandscape: false },
      { image: '/images/showcase/strategy-3.png', rotation: -5, scale: 0.83, isLandscape: true },
      { image: '/images/showcase/strategy-4.png', rotation: 3, scale: 0.87, isLandscape: false },
    ],
  },
];
...
function OrbitingCard({
  ...
  const currentAngleRad = useTransform(
    [baseAngle, slotAngle],
    ([ba, sa]: number[]) => ((ba + sa) * Math.PI) / 180
  );
  ...
  const tilt = useTransform(currentAngleRad, (a) => Math.sin(a) * 10 + card.rotation);
  ...
  const zIndex = useTransform(currentAngleRad, (a) => Math.round(10 + Math.sin(a) * 8));
...
function OrbitRing({ radiusX, radiusY, opacity = 0.06 }: { radiusX: number; radiusY: number; opacity?: number }) {
  return (
    <div
...
export function ShowcaseSection() {
  const [activeKey, setActiveKey] = useState<string>(showcaseData[0].key);
  const [slotAssignments, setSlotAssignments] = useState<number[]>([0, 90, 180, 270]);
  const containerRef = useRef<HTMLDivElement>(null);
  const autoShuffleRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const lastShuffleTimeRef = useRef(0);

  const activeCategory = showcaseData.find((c) => c.key === activeKey) ?? showcaseData[0];
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 md:px-12 lg:px-20">
              ...
              <motion.span
                ...
                style={{
                  ...
                  textShadow: isActive
                    ...
                    : 'none',
                  transition: 'color 0.5s cubic-bezier(0.16,1,0.3,1), opacity 0.5s cubic-bezier(0.16,1,0.3,1), text-shadow 0.5s cubic-bezier(0.16,1,0.3,1), font-size 0.5s cubic-bezier(0.16,1,0.3,1)',
                }}
                onMouseEnter={() => handleMouseEnter(category.key)}
                onClick={() => handleMouseEnter(category.key)}
                onFocus={() => handleFocus(category.key)}
                onBlur={() => { /* keep last active */ }}
                tabIndex={0}
                role="button"
                ...
                whileHover={{
                  opacity: 1,
                  color: '#18181B',
                  textShadow: '0 0 60px rgba(249,115,22,0.2), 0 4px 20px rgba(0,0,0,0.08)',
                }}
                whileTap={{ scale: 0.97 }}
              >
        ...
        >
          ...
          <OrbitRing radiusX={orbitRadius.rx} radiusY={orbitRadius.ry} opacity={0.04} />
          <OrbitRing radiusX={orbitRadius.rx * 0.65} radiusY={orbitRadius.ry * 0.65} opacity={0.025} />
          <OrbitRing radiusX={orbitRadius.rx * 1.3} radiusY={orbitRadius.ry * 1.3} opacity={0.02} />
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div
          ...
          style={{
            ...
            left: `${10 + i * 11}%`,
            top: `${15 + (i % 4) * 20}%`,
            backgroundColor: i % 3 === 0 ? 'rgba(249,115,22,0.15)' : 'rgba(0,0,0,0.04)',
          }}
          animate={{
            y: [0, -15 - i * 4, 0],
            opacity: [0.2, 0.5, 0.2],
